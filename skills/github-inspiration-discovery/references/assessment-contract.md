...
`transferability` scores only how well the project's mechanism can move onto the user's problem. A commitment-device project that is not a focus tool can still score high if the binding mechanism transfers. `boundary_value` scores only whether the project offers a genuinely different approach, not quality, stars, or raw relevance. Do not sort the assessment list yourself. On a contract error, fix the JSON and retry rank; do not start a new search.
