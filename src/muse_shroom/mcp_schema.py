...
HOST_INSTRUCTIONS = (
    ...
    "recall sources and let Muse-shroom own the evidence. Host recall comes first: before "
    "muse_search, search the way you would answer this request without Muse-shroom, boundary "
    "finds included: repositories you already know and your normal Web search, for the core "
    "need and for adjacent or cross-domain directions, as many rounds as you would normally "
    "run. Draft the list you would have given the user. Muse-shroom recall is muse_search, "
    "plus muse_observe and muse_iterate in deep mode; its job is to add what the draft misses. "
    "Do not skip host recall because Muse-shroom is available, do not cut it short because "
    "Muse-shroom will search too, and do not replace muse_search with Web search. Before "
    "muse_rank, pass every draft repository, core-need anchors and boundary finds alike, to "
    ...
    "Default flow: muse_status, host recall, muse_search, then (deep mode) muse_observe and "
    "muse_iterate as next_action requires, then muse_supply with the draft, then muse_rank. "
