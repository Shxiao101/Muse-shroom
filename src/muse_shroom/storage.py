...
class Store:
    ...
    def _migrate(self) -> None:
        ...
        if "request_json" not in snapshot_columns:
            self.db.execute("ALTER TABLE boundary_snapshots ADD COLUMN request_json TEXT")
        self.db.execute(
    ...
    def save_boundary_snapshot(self, search_id: str, stage: str,
                               ...
                               visible_repos: dict[str, Any] | list[str] | None = None,
        ...
        self.db.execute(
            """INSERT INTO boundary_snapshots
               (search_id, stage, boundary_json, delta_json, created_at,
                iteration, hypothesis_json, query_summary_json, repos_json, request_json)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
        ...
        return [
            ...
            for row in self.db.execute(
                """SELECT id,stage,iteration,boundary_json,delta_json,hypothesis_json,
                          query_summary_json,repos_json,request_json,created_at
